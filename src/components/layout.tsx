...
import { PoweroffOutlined, SettingTwoTone, TeamOutlined,ApartmentOutlined } from '@ant-design/icons';
...
class AdminLayout extends React.Component<IAdminLayout>{
  ...
  LeftMenus_Settings = (
    ...
    <Menu.Item key={RouteInfo_Settings.Staffs} icon={<TeamOutlined />}>
        <Link to={RouteInfo_Settings.Staffs}>Staff</Link>
    </Menu.Item>
    <Menu.Item key={RouteInfo_Settings.AdvanceFeatures} icon={<ApartmentOutlined />}>
        <Link to={RouteInfo_Settings.AdvanceFeatures}>Advance Features</Link>
    </Menu.Item>
    </>
  )
      ...
      <Layout>
        <Header className="header">
          ...
          <Menu theme="light" mode="horizontal" onClick={this.handleClick} selectedKeys={[current]} defaultSelectedKeys={[RouteInfo_Dashboard.MenuInfo]}>
            ...
            <Menu.Item key="logout" style={{ padding: '0 0 0 80%' }} onClick={this.logout.bind(this)}> <PoweroffOutlined></PoweroffOutlined></Menu.Item>
